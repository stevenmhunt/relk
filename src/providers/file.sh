...
relk_platform_provider_file_get_key_value() {
    ...
    while IFS="${DELIM_COL}" read -r file_namespace file_key file_value file_value_type file_attributes file_constraints; do
        ...
        if $match_found; then
            results+=("$file_value${DELIM_COL}$file_value_type${DELIM_COL}$file_attributes${DELIM_COL}$constraint_count")
    ...
    echo "$final_result|$final_result_type|$final_result_attributes"
}
